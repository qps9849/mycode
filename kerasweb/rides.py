...
def result():
    ...
    test_set = np.array([week, child, distance, rides, games, clean]).reshape(1, 7)
    ...
    return render_template('rides/result.html',
                           rate='{:.2f}%'.format(rate[0][0] * 100), result=result, weekend = weekend, child = child,
                          distance = distance, rides = rides, games = games, wait = wait, clean = clean)
